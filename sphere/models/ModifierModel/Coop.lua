...
function Coop:apply(config, chart)
	...
	local move = false
	local currentTime
	for _, n in chart.notes:iter() do
		if (n.noteType == "ShortNote" or n.noteType == "LongNoteStart") then
			if not currentTime or currentTime + config.value < n:getTime() then
				currentTime = n:getTime()
				move = not move
			end
			if move then
				local type, new_column = InputMode:splitInput(n.column)
