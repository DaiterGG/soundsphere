...
function Shuffle:apply(config, noteChart)
	...
	local count = seed
	for _, n in ipairs(noteChart.notes:getLinkedNotes()) do
		if
			n.startNote.type == "note" or
			n.startNote.type == "hold"
		then
			if count >= 10 then
				count = 1
			else
			...
			if persentage >= count and count > 0 then
				...
				n:setColumn("key" .. rngIndex)
			end
		end
		new_notes:insertLinked(n)
	end
	noteChart.notes = new_notes
