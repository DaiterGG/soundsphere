...
function InputNote:new(column, width, scrollSpeed, color, startPosY, startPosX, vertDirection)
    ...
    if vertDirection == "upscroll" then new.direction = -1
    else new.direction = 1 end
...
function InputNote:update()
    ...
    local res = sh / 1080
    local x = self.posX * res
    local y = (self.posY - self.height) * res
